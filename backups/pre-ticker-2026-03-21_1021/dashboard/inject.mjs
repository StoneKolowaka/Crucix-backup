...
function sumAirHotspots(hotspots = []) {
  return hotspots.reduce(
    (sum, hotspot) => sum + (hotspot.totalAircraft || 0),
    0,
  );
}
...
function summarizeAirHotspots(hotspots = []) {
  return hotspots.map((h) => ({
    ...
    top: Object.entries(h.byCountry || {})
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5),
  }));
}
...
function loadOpenSkyFallback(currentTimestamp) {
  ...
  const currentMs = currentTimestamp
  ...
  for (const file of files) {
    ...
    try {
      ...
      const priorTimestamp =
        prior.sources?.OpenSky?.timestamp || prior.crucix?.timestamp || null;
      if (
        priorTimestamp &&
        Number.isFinite(currentMs) &&
        new Date(priorTimestamp).getTime() >= currentMs
      )
        continue;
  ...
  try {
    ...
    while ((match = itemRegex.exec(xml)) !== null) {
      ...
      if (title && title !== source)
        items.push({ title, date: pubDate, source, url: link || undefined });
    }
    return items;
  } catch (e) {
...
const RSS_SOURCE_FALLBACKS = {
  ...
  Teslarati: { lat: 37.3944, lon: -122.1503, region: "North America" },
  Electrek: { lat: 40.7128, lon: -74.006, region: "North America" },
  SpaceNews: { lat: 38.9072, lon: -77.0369, region: "North America" },
  TechCrunch: { lat: 37.7749, lon: -122.4194, region: "North America" },
};
...
export function generateIdeas(V2) {
  ...
  if (vix && vix.value > 20) {
    ideas.push({
      ...
      type: "hedge",
      confidence: vix.value > 25 ? "High" : "Medium",
      horizon: "tactical",
    });
  }
  ...
  if (spread) {
    ideas.push({
      title:
        spread.value > 0 ? "Yield Curve Normalizing" : "Yield Curve Inverted",
  ...
  const unemployment = V2.bls.find(
    (b) => b.id === "LNS14000000" || b.id === "UNRATE",
  );
  const payrolls = V2.bls.find(
    (b) => b.id === "CES0000000001" || b.id === "PAYEMS",
  );
  if (spread && unemployment && payrolls) {
    const weakLabor =
      unemployment.value > 4.3 ||
      (payrolls.momChange && payrolls.momChange < -50);
  ...
  if (conflictEvents > 50 && V2.energy.wtiRecent.length > 1) {
    const wtiMove =
      V2.energy.wtiRecent[0] -
      V2.energy.wtiRecent[V2.energy.wtiRecent.length - 1];
    if (wtiMove > 2) {
      ideas.push({
  ...
  const ppi = V2.bls.find(
    (b) => b.id === "WPUFD49104" || b.id === "PCU--PCU--",
  );
...
export async function synthesize(data) {
  ...
  const airFallback =
    sumAirHotspots(liveAirHotspots) > 0
      ? null
      : loadOpenSkyFallback(
          data.sources.OpenSky?.timestamp || data.crucix?.timestamp,
        );
  ...
  const thermal = (data.sources.FIRMS?.hotspots || []).map((h) => ({
    region: h.region,
    det: h.totalDetections || 0,
    night: h.nightDetections || 0,
    hc: h.highConfidence || 0,
    fires: (h.highIntensity || [])
      .slice(0, 8)
      .map((f) => ({ lat: f.lat, lon: f.lon, frp: f.frp || 0 })),
  }));
  const tSignals = data.sources.FIRMS?.signals || [];
  const chokepoints = Object.values(
    data.sources.Maritime?.chokepoints || {},
  ).map((c) => ({
  ...
  const nuke = (data.sources.Safecast?.sites || []).map((s) => ({
    site: s.site,
    anom: s.anomaly || false,
    cpm: s.avgCPM,
    n: s.recentReadings || 0,
  }));
  ...
  const sdrZones = Object.values(sdrConflict).map((z) => ({
    region: z.region,
    count: z.count || 0,
    receivers: (z.receivers || [])
      .slice(0, 5)
      .map((r) => ({ name: r.name || "", lat: r.lat || 0, lon: r.lon || 0 })),
  }));
  ...
  const fred = (data.sources.FRED?.indicators || []).map((f) => ({
    ...
    date: f.date,
    recent: f.recent || [],
    momChange: f.momChange,
    momChangePct: f.momChangePct,
  }));
  ...
  const treasury = {
    totalDebt: debtArr[0]?.totalDebt || "0",
    signals: treasuryData.signals || [],
  };
  const gscpi = data.sources.GSCPI?.latest || null;
  const defense = (data.sources.USAspending?.recentDefenseContracts || [])
    .slice(0, 5)
    .map((c) => ({
      recipient: c.recipient?.substring(0, 40),
      amount: c.amount,
      desc: c.description?.substring(0, 80),
    }));
  const noaa = {
    totalAlerts: data.sources.NOAA?.totalSevereAlerts || 0,
    alerts: (data.sources.NOAA?.topAlerts || [])
      .filter((a) => a.lat != null && a.lon != null)
      .slice(0, 10)
      .map((a) => ({
        event: a.event,
        severity: a.severity,
        headline: a.headline?.substring(0, 120),
        lat: a.lat,
        lon: a.lon,
      })),
  };
  ...
  const epa = {
    totalReadings: epaData.totalReadings || 0,
    stations: epaStations.slice(0, 10),
  };
  ...
  const spaceStations = (spaceData.spaceStations || [])
    .map((s) => estimateSatPosition(s))
    .filter(Boolean);
  const space = {
  ...
  const gdelt = {
    ...
    topTitles: (gdeltData.allArticles || [])
      .slice(0, 5)
      .map((a) => a.title?.substring(0, 80)),
  ...
  if (yfWti?.history?.length)
    energy.wtiRecent = yfWti.history.map((h) => h.close);

  // === Top 200 Crypto watchlist (CoinGecko) ===
  const c200Raw = data.sources.Crypto200 || {};
  const crypto200 = {
    coins: (c200Raw.coins || []).map((c) => ({
      rank: c.rank,
      symbol: c.symbol,
      name: c.name,
      price: c.price,
    ...
    summary: c200Raw.summary || null,
    gainers24h: c200Raw.gainers24h || [],
    losers24h: c200Raw.losers24h || [],
    highestVolume: c200Raw.highestVolume || [],
  };
  ...
  const V2 = {
    ...
    airMeta: {
      ...
      timestamp:
        airFallback?.timestamp ||
        data.sources.OpenSky?.timestamp ||
        data.crucix?.timestamp ||
        null,
      source: airFallback ? "OpenSky fallback" : "OpenSky",
      ...(airFallback ? { fallbackFile: airFallback.file } : {}),
      ...(data.sources.OpenSky?.error
        ? { error: data.sources.OpenSky.error }
        : {}),
    },
    sdr: {
      total: sdrNet.totalReceivers || 0,
      online: sdrNet.online || 0,
      zones: sdrZones,
    },
    ...
    crypto200, // Top 200 crypto watchlist (CoinGecko, no API key)
    ideas: [],
...
function buildNewsFeed(rssNews, gdeltData, tgUrgent, tgTop) {
  ...
  for (const a of (gdeltData.allArticles || []).slice(0, 10)) {
    if (a.title) {
      ...
      feed.push({
        headline: a.title.substring(0, 100),
        source: "GDELT",
        type: "gdelt",
  ...
  const selectedKeys = new Set();
  const keyFor = (item) =>
    `${item.type}|${item.source}|${item.headline}|${item.timestamp}`;
  const pushUnique = (item) => {
    const key = keyFor(item);
    if (selectedKeys.has(key)) return;
    selected.push(item);
