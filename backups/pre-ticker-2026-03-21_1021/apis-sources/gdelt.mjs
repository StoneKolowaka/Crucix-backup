...
export async function searchEvents(query = "", opts = {}) {
  ...
  const q =
    query || "conflict OR crisis OR military OR sanctions OR war OR economy";
...
export async function briefing() {
  ...
  const all = await searchEvents(
    "conflict OR military OR economy OR crisis OR war OR sanctions OR tariff OR strike OR outbreak OR Tesla OR SpaceX OR Musk OR xAI OR Starlink",
  ...
  let geoPoints = [];
  try {
    const geo = await geoEvents("conflict OR military OR protest OR crisis", {
      maxPoints: 30,
      timespan: "24h",
    });
    ...
    /* geo endpoint optional — don't break briefing */
  }
